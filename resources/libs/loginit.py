...
def updateLogin(do, who):
	...
	if do == 'update':
		...
		#else: wiz.LogNotify(name,'login Data: [COLOR darkgoldenrod]Not Found![/COLOR]', 2000, icon)
	elif do == 'clearaddon':
	...
	if who == 'all':
		...
		if os.path.exists(LOGINID[who]['path']):
			...
			if u == None or u == '': return
				...
				if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to save the [COLOR %s]Login[/COLOR] data for [COLOR %s]%s[/COLOR]?" % (COLOR2, COLOR1, COLOR1, n), "Addon: [COLOR darkgoldenrod][B]%s[/B][/COLOR]" % u, "Saved:[/COLOR] [COLOR darkgoldenrod][B]%s[/B][/COLOR]" % su if not su == '' else 'Saved:[/COLOR] [COLOR darkgoldenrod][B]None[/B][/COLOR]', yeslabel="[B][COLOR darkgoldenrod]Save Data[/COLOR][/B]", nolabel="[B][COLOR darkgoldenrod]No Cancel[/COLOR][/B]"):
					loginIt('update', who)
...
def importlist(who):
	if who == 'all':
		...
		if os.path.exists(LOGINID[who]['file']):
			...
			if len(m) > 0:
				if not m[0] == su:
					if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to import the [COLOR %s]Login[/COLOR] data for [COLOR %s]%s[/COLOR]?" % (COLOR2, COLOR1, COLOR1, n), "File: [COLOR darkgoldenrod][B]%s[/B][/COLOR]" % m[0], "Saved:[/COLOR] [COLOR darkgoldenrod][B]%s[/B][/COLOR]" % su if not su == '' else 'Saved:[/COLOR] [COLOR darkgoldenrod][B]None[/B][/COLOR]', yeslabel="[B][COLOR darkgoldenrod]Save Data[/COLOR][/B]", nolabel="[B][COLOR darkgoldenrod]No Cancel[/COLOR][/B]"):
						wiz.setS(sa, m[0])
