	...
	class MyWindow(xbmcgui.WindowDialog):
		...
		def doWrite(self):
			...
			if os.path.exists(ADVANCED):
				choice = DIALOG.yesno(ADDONTITLE, "[COLOR %s]There is currently an active [COLOR %s]AdvancedSettings.xml[/COLOR], would you like to remove it and continue?[/COLOR]" % (COLOR2, COLOR1), yeslabel="[B][COLOR darkgoldenrod]Remove Settings[/COLOR][/B]", nolabel="[B][COLOR darkgoldenrod]Cancel Write[/COLOR][/B]")
				if choice == 0: return
