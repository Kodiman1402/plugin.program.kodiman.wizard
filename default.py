...
def index():
	if AUTOUPDATE == 'Yes':
		if wiz.workingURL(WIZARDFILE) == True:
			...
			if ver > VERSION: addFile('%s [v%s] [COLOR darkgoldenrod][B][UPDATE v%s][/B][/COLOR]' % (ADDONTITLE, VERSION, ver), 'wizardupdate', themeit=THEME2)
	...
	if len(BUILDNAME) > 0:
		...
		if version > BUILDVERSION: build = '%s [COLOR darkgoldenrod][B][UPDATE v%s][/B][/COLOR]' % (build, version)
...
def viewBuild(name):
	...
	for version, url, gui, kodi, themefile, icon, fanart, preview, adult, description in match:
		...
		if BUILDNAME == name and version > BUILDVERSION:
			build = '%s [COLOR darkgoldenrod][Aktuelle v%s][/COLOR]' % (build, BUILDVERSION)
					...
					if plugin.lower() == 'section':
						...
						try:
							...
							if os.path.exists(add):
								name   = "[COLOR ghostwhite][Installiert][/COLOR] %s" % name
						except:
							pass
						x += 1
				...
				for name, url, repository, repositoryxml, repositoryurl, icon, fanart, adult, description in match:
					if os.path.exists(os.path.join(ADDONS, plugin)):
						...
						if selected == 0:
							...
							if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du die Addon Daten entfernen:" % COLOR2, "[COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR1, plugin), yeslabel="[B][COLOR ghostwhite]JA, Entfernen[/COLOR][/B]", nolabel="[B][COLOR darkgoldenrod]NEIN, abbrechen[/COLOR][/B]"):
					...
					if not repository.lower() == 'none' and not os.path.exists(repo):
						...
						if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du die Repository installieren [COLOR %s]%s[/COLOR]:" % (COLOR2, COLOR1, plugin), "[COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR1, repository), yeslabel="[B][COLOR ghostwhite]JA, installieren[/COLOR][/B]", nolabel="[B][COLOR darkgoldenrod]NEIN, abbrechen[/COLOR][/B]"): 
...
def maintMenu(view=None):
	on = '[B][COLOR ghostwhite]AN[/COLOR][/B]'; off = '[B][COLOR darkgoldenrod]AUS[/COLOR][/B]'
	...
	wizlogsize = ': [COLOR darkgoldenrod]Not Found[/COLOR]' if not os.path.exists(WIZLOG) else ": [COLOR ghostwhite]%s[/COLOR]" % wiz.convertSize(os.path.getsize(WIZLOG))
	if includeall == 'true':
		includelas = 'true'
		includecov = 'true'
		includeexo = 'true'
	...
	if view == "clean" or SHOWMAINT == 'true': 
		addFile('Komplette Reinigung: [COLOR ghostwhite][B]%s[/B][/COLOR]' % wiz.convertSize(totalsize)  ,'fullclean',       icon=ICONMAINT, themeit=THEME3)
		addFile('Cache reinigen: [COLOR ghostwhite][B]%s[/B][/COLOR]' % wiz.convertSize(sizecache)     ,'clearcache',      icon=ICONMAINT, themeit=THEME3)
		addFile('Packages reinigen: [COLOR ghostwhite][B]%s[/B][/COLOR]' % wiz.convertSize(sizepack)   ,'clearpackages',   icon=ICONMAINT, themeit=THEME3)
		addFile('Thumbnails reinigen: [COLOR ghostwhite][B]%s[/B][/COLOR]' % wiz.convertSize(sizethumb),'clearthumb',      icon=ICONMAINT, themeit=THEME3)
		...
		addFile('[COLOR darkgoldenrod]Frischer Start[/COLOR]',                    'freshstart',      icon=ICONMAINT, themeit=THEME3)
	...
	if autoclean == 'true':
		addFile('--- Auto Reinigung Intervall: [B][COLOR ghostwhite]%s[/COLOR][/B]' % feq[AUTOFEQ], 'changefeq', icon=ICONMAINT, themeit=THEME3)
...
def writeAdvanced(name, url):
	ADVANCEDWORKING = wiz.workingURL(url)
	if ADVANCEDWORKING == True:
		if os.path.exists(ADVANCED): choice = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du deine aktuelle AdvancedSettings ersetzen [COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, name), yeslabel="[B][COLOR ghostwhite]JA, Ersetzen[/COLOR][/B]", nolabel="[B][COLOR darkgoldenrod]NEIN, Abbrechen[/COLOR][/B]")
		else: choice = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du herunter laden und installieren [COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, name), yeslabel="[B][COLOR ghostwhite]Installieren[/COLOR][/B]", nolabel="[B][COLOR darkgoldenrod]Abbrechen[/COLOR][/B]")
...
def saveMenu():
	on = '[COLOR ghostwhite]AN[/COLOR]'; off = '[COLOR darkgoldenrod]AUS[/COLOR]'
...
def traktMenu():
	trakt = '[COLOR ghostwhite]AN[/COLOR]' if KEEPTRAKT == 'true' else '[COLOR darkgoldenrod]AUS[/COLOR]'
	...
	for trakt in traktit.ORDER:
		...
		if not os.path.exists(path): addFile('[COLOR darkgoldenrod]Addon Daten: Nicht installiert[/COLOR]', '', icon=icon, fanart=fanart, menu=menu)
		elif not auser:              addFile('[COLOR darkgoldenrod]Addon Daten: Nicht registriert[/COLOR]','authtrakt', trakt, icon=icon, fanart=fanart, menu=menu)
		else:                        addFile('[COLOR ghostwhite]Addon Daten: %s[/COLOR]' % auser,'authtrakt', trakt, icon=icon, fanart=fanart, menu=menu)
		if user == "":
			if os.path.exists(file): addFile('[COLOR darkgoldenrod]Gespeicherte Daten: Gespeicherte Datei gefunden(importiere Daten)[/COLOR]','importtrakt', trakt, icon=icon, fanart=fanart, menu=menu2)
			else :                   addFile('[COLOR darkgoldenrod]Gespeicherte Daten: Nicht gespeichert[/COLOR]','savetrakt', trakt, icon=icon, fanart=fanart, menu=menu2)
		else:                        addFile('[COLOR ghostwhite]Gespeicherte Daten: %s[/COLOR]' % user, '', icon=icon, fanart=fanart, menu=menu2)
...
def realMenu():
	real = '[COLOR ghostwhite]AN[/COLOR]' if KEEPREAL == 'true' else '[COLOR darkgoldenrod]AUS[/COLOR]'
	...
	for debrid in debridit.ORDER:
		...
		addFile('[+]-> %s' % name,     '', icon=icon, fanart=fanart, themeit=THEME3)
		if not os.path.exists(path): addFile('[COLOR darkgoldenrod]Addon Daten: Nicht installiert[/COLOR]', '', icon=icon, fanart=fanart, menu=menu)
		elif not auser:              addFile('[COLOR darkgoldenrod]Addon Daten: Nicht registriert[/COLOR]','authdebrid', debrid, icon=icon, fanart=fanart, menu=menu)
		else:                        addFile('[COLOR ghostwhite]Addon Daten: %s[/COLOR]' % auser,'authdebrid', debrid, icon=icon, fanart=fanart, menu=menu)
		if user == "":
			if os.path.exists(file): addFile('[COLOR darkgoldenrod]Gespeicherte Daten: Gespeicherte Datei gefunden(importiere Daten)[/COLOR]','importdebrid', debrid, icon=icon, fanart=fanart, menu=menu2)
			else :                   addFile('[COLOR darkgoldenrod]Gespeicherte Daten: Nicht gespeichert[/COLOR]','savedebrid', debrid, icon=icon, fanart=fanart, menu=menu2)
		else:                        addFile('[COLOR ghostwhite]Gespeicherte Daten: %s[/COLOR]' % user, '', icon=icon, fanart=fanart, menu=menu2)
...
def loginMenu():
	login = '[COLOR ghostwhite]AN[/COLOR]' if KEEPLOGIN == 'true' else '[COLOR darkgoldenrod]AUS[/COLOR]'
	...
	for login in loginit.ORDER:
		...
		addFile('[+]-> %s' % name,     '', icon=icon, fanart=fanart, themeit=THEME3)
		if not os.path.exists(path): addFile('[COLOR darkgoldenrod]Addon Data: Not Installed[/COLOR]', '', icon=icon, fanart=fanart, menu=menu)
		elif not auser:              addFile('[COLOR darkgoldenrod]Addon Data: Not Registered[/COLOR]','authlogin', login, icon=icon, fanart=fanart, menu=menu)
		else:                        addFile('[COLOR darkgoldenrod]Addon Data: %s[/COLOR]' % auser,'authlogin', login, icon=icon, fanart=fanart, menu=menu)
		if user == "":
			if os.path.exists(file): addFile('[COLOR darkgoldenrod]Saved Data: Save File Found(Import Data)[/COLOR]','importlogin', login, icon=icon, fanart=fanart, menu=menu2)
			else :                   addFile('[COLOR darkgoldenrod]Saved Data: Not Saved[/COLOR]','savelogin', login, icon=icon, fanart=fanart, menu=menu2)
		else:                        addFile('[COLOR darkgoldenrod]Saved Data: %s[/COLOR]' % user, '', icon=icon, fanart=fanart, menu=menu2)
...
def removeAddonMenu():
	...
	if len(selected) > 0:
		...
		else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du [COLOR %s]KODI schliessen[/COLOR] oder [COLOR %s]Profil neu laden[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR ghostwhite]Profil neu laden[/COLOR][/B]", nolabel="[B][COLOR darkgoldenrod]KODI schliessen[/COLOR][/B]")
...
def removeAddonDataMenu():
	if os.path.exists(ADDOND):
		addFile('[COLOR darkgoldenrod][B][Entferne][/B][/COLOR] Alle Addon_Daten', 'removedata', 'all', themeit=THEME2)
		addFile('[COLOR darkgoldenrod][B][Entferne][/B][/COLOR] Alle Addon_Daten der deinstallierten Addons', 'removedata', 'uninstalled', themeit=THEME2)
		addFile('[COLOR darkgoldenrod][B][Entferne][/B][/COLOR] Alle leeren Ordner in den Addon_Daten', 'removedata', 'empty', themeit=THEME2)
		addFile('[COLOR darkgoldenrod][B][Entferne][/B][/COLOR] %s Addon_Daten' % ADDONTITLE, 'resetaddon', themeit=THEME2)
		if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
		...
		for folder in sorted(fold, key = lambda x: x):
			...
			replace = {'audio.':'[COLOR orange][AUDIO] [/COLOR]', 'metadata.':'[COLOR cyan][METADATA] [/COLOR]', 'module.':'[COLOR orange][MODULE] [/COLOR]', 'plugin.':'[COLOR blue][PLUGIN] [/COLOR]', 'program.':'[COLOR orange][PROGRAM] [/COLOR]', 'repository.':'[COLOR gold][REPO] [/COLOR]', 'script.':'[COLOR darkgoldenrod][SCRIPT] [/COLOR]', 'service.':'[COLOR darkgoldenrod][SERVICE] [/COLOR]', 'skin.':'[COLOR dodgerblue][SKIN] [/COLOR]', 'video.':'[COLOR orange][VIDEO] [/COLOR]', 'weather.':'[COLOR yellow][WEATHER] [/COLOR]'}
			for rep in replace:
				folderdisplay = folderdisplay.replace(rep, replace[rep])
			if foldername in EXCLUDES: folderdisplay = '[COLOR ghostwhite][B][Gesichert][/B][/COLOR] %s' % folderdisplay
			else: folderdisplay = '[COLOR darkgoldenrod][B][Entfernen][/B][/COLOR] %s' % folderdisplay
...
def enableAddons():
	addFile("[I][B][COLOR darkgoldenrod]!!Hinweis: Die Deaktivierung mancher Addons kann Probleme verursachen!![/COLOR][/B][/I]", '', icon=ICONMAINT)
	fold = glob.glob(os.path.join(ADDONS, '*/'))
	x = 0
	for folder in sorted(fold, key = lambda x: x):
		foldername = os.path.split(folder[:-1])[1]
		...
		if os.path.exists(addonxml):
			...
			try:
				...
				state  = "[COLOR ghostwhite][Aktiviert][/COLOR]"
				goto   = "false"
			except:
				state  = "[COLOR darkgoldenrod][Deaktiviert][/COLOR]"
				goto   = "true"
				pass
...
def buildWizard(name, type, theme=None, over=False):
	...
	if type == 'gui':
		if name == BUILDNAME:
			if over == True: yes = 1
			else: yes = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Willst du den GuiFix anwenden:' % COLOR2, '[COLOR %s]%s[/COLOR]?[/COLOR]' % (COLOR1, name), nolabel='[B][COLOR darkgoldenrod]NEIN, Abbrechen[/COLOR][/B]',yeslabel='[B][COLOR ghostwhite]JA, Fix anwenden[/COLOR][/B]')
		else: 
			yes = DIALOG.yesno("%s - [COLOR darkgoldenrod]WARNUNG!![/COLOR]" % ADDONTITLE, "[COLOR %s][COLOR %s]%s[/COLOR] Build ist aktuell nicht installiert." % (COLOR2, COLOR1, name), "Willst du den GuiFix trotzdem anwenden?.[/COLOR]", nolabel='[B][COLOR darkgoldenrod]NEIN, Abbrechen[/COLOR][/B]',yeslabel='[B][COLOR ghostwhite]JA, Fix anwenden[/COLOR][/B]')
		if yes:
			...
			else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Der GuiFix wurde installiert.  Willst du das Profil neu laden oder KODI schliessen?[/COLOR]" % COLOR2, yeslabel="[B][COLOR darkgoldenrod]Profil neu laden[/COLOR][/B]", nolabel="[B][COLOR ghostwhite]KODI schliessen[/COLOR][/B]")
		...
		if warning == True:
			yes_pressed = DIALOG.yesno("%s - [COLOR darkgoldenrod]WARNUNG!![/COLOR]" % ADDONTITLE, '[COLOR %s]Es besteht die Moglichkeit das es Darstellungsprobleme des Skins gibt' % COLOR2, 'Bei der Installation eines %s Builds auf einer KODI %s Installation' % (wiz.checkBuild(name, 'kodi'), KODIV), 'Willst du trotzdem installieren: [COLOR %s]%s v%s[/COLOR]?[/COLOR]' % (COLOR1, name, wiz.checkBuild(name,'version')), nolabel='[B][COLOR darkgoldenrod]NEIN, Abbrechen[/COLOR][/B]',yeslabel='[B][COLOR ghostwhite]JA, Installieren[/COLOR][/B]')
		else:
			if not over == False: yes_pressed = 1
			else: yes_pressed = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Willst du herunterladen und installieren:' % COLOR2, '[COLOR %s]%s v%s[/COLOR]?[/COLOR]' % (COLOR1, name, wiz.checkBuild(name,'version')), nolabel='[B][COLOR darkgoldenrod]NEIN, Abbrechen[/COLOR][/B]',yeslabel='[B][COLOR ghostwhite]JA, Installieren[/COLOR][/B]')
		if yes_pressed:
			wiz.clearS('build')
			...
			if int(float(percent)) > 0:
				...
				if int(float(errors)) > 0:
					yes=DIALOG.yesno(ADDONTITLE, '[COLOR %s][COLOR %s]%s v%s[/COLOR]' % (COLOR2, COLOR1, name, wiz.checkBuild( name,'version')), 'Erledigt: [COLOR %s]%s%s[/COLOR] [Fehler:[COLOR %s]%s[/COLOR]]' % (COLOR1, percent, '%', COLOR1, errors), 'Willst du die Fehler sehen?[/COLOR]', nolabel='[B][COLOR darkgoldenrod]NEIN Danke[/COLOR][/B]', yeslabel='[B][COLOR ghostwhite]Zeige die Fehler[/COLOR][/B]')
					if yes:
				...
				else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du [COLOR %s]KODI schliessen[/COLOR] oder [COLOR %s]Profil neu laden[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR darkgoldenrod]Profil neu laden[/COLOR][/B]", nolabel="[B][COLOR ghostwhite]KODI schliessen[/COLOR][/B]")
				if todo == 1: wiz.reloadFix()
		...
		if theme == None:
			...
			if not themefile == 'http://' and wiz.workingURL(themefile) == True:
				...
				if len(themelist) > 0:
					if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Diese Build [COLOR %s]%s[/COLOR] gibt es mit [COLOR %s]%s[/COLOR] verschiedenen Themen" % (COLOR2, COLOR1, name, COLOR1, len(themelist)), "Willst du es jetzt installieren?[/COLOR]", yeslabel="[B][COLOR ghostwhite]Installiere Theme[/COLOR][/B]", nolabel="[B][COLOR darkgoldenrod]Abbrechen[/COLOR][/B]"):
		...
		else: installtheme = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Willst du das Theme installieren:' % COLOR2, '[COLOR %s]%s[/COLOR]' % (COLOR1, theme), 'fur [COLOR %s]%s v%s[/COLOR]?[/COLOR]' % (COLOR1, name, wiz.checkBuild(name,'version')), yeslabel="[B][COLOR ghostwhite]Installiere Theme[/COLOR][/B]", nolabel="[B][COLOR darkgoldenrod]Abbrechen[/COLOR][/B]")
		if installtheme:
			...
			if url not in ["fresh", "normal"]:
				...
				if test == True:
					...
					#if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Installing the theme [COLOR %s]%s[/COLOR] requires the skin to be swaped back to [COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, theme, COLOR1, skin[5:]), "Would you like to switch the skin?[/COLOR]", yeslabel="[B][COLOR ghostwhite]Switch Skin[/COLOR][/B]", nolabel="[B][COLOR darkgoldenrod]Don't Switch[/COLOR][/B]"):
...
def thirdPartyInstall(name, url):
	...
	type = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du eine [COLOR %s]Frische Installation[/COLOR] oder eine [COLOR %s]Normale Installation[/COLOR] for:[/COLOR]" % (COLOR2, COLOR1, COLOR1), "[COLOR %s]%s[/COLOR]" % (COLOR1, name), yeslabel="[B][COLOR ghostwhite]Frische Installation[/COLOR][/B]", nolabel="[B][COLOR darkgoldenrod]Normale Installation[/COLOR][/B]")
	if type == 1:
	...
	if int(float(percent)) > 0:
		...
		if int(float(errors)) > 0:
			yes=DIALOG.yesno(ADDONTITLE, '[COLOR %s][COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, name), 'Erledigt: [COLOR %s]%s%s[/COLOR] [Fehler:[COLOR %s]%s[/COLOR]]' % (COLOR1, percent, '%', COLOR1, errors), 'Willst du die Fehler sehen?[/COLOR]', nolabel='[B][COLOR darkgoldenrod]NEIN, Danke[/COLOR][/B]',yeslabel='[B][COLOR ghostwhite]Zeige Fehler[/COLOR][/B]')
			if yes:
				if isinstance(errors, unicode):
					error = error.encode('utf-8')
				wiz.TextBox(ADDONTITLE, error)
...
def apkInstaller(apk, url):
	...
	if wiz.platform() == 'android':
		yes = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du herunterladen und installieren:" % COLOR2, "[COLOR %s]%s[/COLOR]" % (COLOR1, apk), yeslabel="[B][COLOR ghostwhite]Download[/COLOR][/B]", nolabel="[B][COLOR darkgoldenrod]Abbrechen[/COLOR][/B]")
		...
		wiz.ebi('StartAndroidActivity("org.xbmc.kodi","android.intent.action.VIEW","application/vnd.android.package-archive","file:'+lib+'")')
			...
			if len(match) > 0:
				...
				for item in match:
					if all == None and not ADDON_ID in item: continue
						...
						x += 1
						msg += "[COLOR darkgoldenrod]Fehler Nummer %s[/COLOR]\n(PythonToCppException) : -->Python callback/script returned the following error<--%s-->End of Python script error report<--\n\n" % (x, item.replace('                                          ', '\n').replace('\\\\','\\').replace(HOME, ''))
				if x > 0:
...
def removeAddon(addon, name, over=False):
	if not over == False:
		...
		yes = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Willst du das Addon wirklich entfernen:'% COLOR2, 'Name: [COLOR %s]%s[/COLOR]' % (COLOR1, name), 'ID: [COLOR %s]%s[/COLOR][/COLOR]' % (COLOR1, addon), yeslabel='[B][COLOR ghostwhite]JA, entferne Addon[/COLOR][/B]', nolabel='[B][COLOR darkgoldenrod]NEIN, abbrechen[/COLOR][/B]')
	if yes == 1:
...
def removeAddonData(addon, name=None, over=False):
	if addon == 'all':
		if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Willst du [COLOR %s]ALLE[/COLOR] Addon-Daten entfernen die im User_Data Ordner gespeichert sind?[/COLOR]' % (COLOR2, COLOR1), yeslabel='[B][COLOR ghostwhite]JA, entferne Daten[/COLOR][/B]', nolabel='[B][COLOR darkgoldenrod]NEIN, abbrechen[/COLOR][/B]'):
		...
		if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Willst du [COLOR %s]ALLE[/COLOR] Addon-Daten entfernen die im User-Data Ordner gespeichert sind?[/COLOR]' % (COLOR2, COLOR1), yeslabel='[B][COLOR ghostwhite]JA, entferne Daten[/COLOR][/B]', nolabel='[B][COLOR darkgoldenrod]NEIN, abbrechen[/COLOR][/B]'):
			total = 0
		...
		if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Willst du [COLOR %s]ALLE[/COLOR] leeren Ordner im User_Data Ordner entfernen?[/COLOR]' % (COLOR2, COLOR1), yeslabel='[B][COLOR ghostwhite]JA, entferne Ordner[/COLOR][/B]', nolabel='[B][COLOR darkgoldenrod]NEIN, abbrechen[/COLOR][/B]'):
		...
		if addon in EXCLUDES:
			...
			if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Willst du auch die Addon-Daten entfernen von:[/COLOR]' % COLOR2, '[COLOR %s]%s[/COLOR]' % (COLOR1, addon), yeslabel='[B][COLOR ghostwhite]JA, entferne Daten[/COLOR][/B]', nolabel='[B][COLOR darkgoldenrod]NEIN, abbrechen[/COLOR][/B]'):
		...
		if os.path.exists(trakt):
			...
			for item in files:
				...
				if os.path.exists(old):
					if not DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du die aktuelle [COLOR %s]%s[/COLOR] Datei ersetzen?" % (COLOR2, COLOR1, item), yeslabel="[B][COLOR ghostwhite]JA, ersetzen[/COLOR][/B]", nolabel="[B][COLOR darkgoldenrod]NEIN, abbrechen[/COLOR][/B]"): continue
					else: os.remove(old)
...
def freshStart(install=None, over=False):
	...
	elif install == 'restore': yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du KODI resetten" % COLOR2, "auf standard Einstellungen", "bevor du das lokale Backup installierst?[/COLOR]", nolabel='[B][COLOR darkgoldenrod]NEIN, abbrechen[/COLOR][/B]', yeslabel='[B][COLOR ghostwhite]JA, weiter[/COLOR][/B]')
	elif install: yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du KODI resetten" % COLOR2, "auf standard Einstellungen", "bevor du [COLOR %s]%s[/COLOR] installierst?" % (COLOR1, install), nolabel='[B][COLOR darkgoldenrod]NEIN, abbrechen[/COLOR][/B]', yeslabel='[B][COLOR ghostwhite]JA, weiter[/COLOR][/B]')
	else: yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du KODI resetten" % COLOR2, "auf standard Einstellungen?[/COLOR]", nolabel='[B][COLOR darkgoldenrod]NEIN, abbrechen[/COLOR][/B]', yeslabel='[B][COLOR ghostwhite]JA, weiter[/COLOR][/B]')
	if yes_pressed:
		if not wiz.currSkin() in ['skin.confluence', 'skin.estuary']:
			skin = 'skin.confluence' if KODIV < 17 else 'skin.estuary'
			#yes=DIALOG.yesno(ADDONTITLE, "[COLOR %s]The skin needs to be set back to [COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, skin[5:]), "Before doing a fresh install to clear all Texture files,", "Would you like us to do that for you?[/COLOR]", yeslabel="[B][COLOR ghostwhite]Switch Skins[/COLOR][/B]", nolabel="[B][COLOR darkgoldenrod]I'll Do It[/COLOR][/B]";
			#if yes:
			skinSwitch.swapSkins(skin)
			x = 0
			xbmc.sleep(1000)
...
####THANKS GUYS @ NaN #######
def clearCache():
	if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Willst du den Cache reinigen?[/COLOR]' % COLOR2, nolabel='[B][COLOR darkgoldenrod]NEIN, abbrechen[/COLOR][/B]', yeslabel='[B][COLOR ghostwhite]JA, reinigen[/COLOR][/B]'):
		wiz.clearCache()
		#clearThumb()

def totalClean():
	if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Willst du den Cache, Packages und Thumbnails reinigen?[/COLOR]' % COLOR2, nolabel='[B][COLOR darkgoldenrod]NEIN, abbrechen[/COLOR][/B]',yeslabel='[B][COLOR ghostwhite]JA, alle reinigen[/COLOR][/B]'):
		wiz.clearCache()
...
def clearThumb(type=None):
	...
	else: choice = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Willst du %s und Thumbnails Ordner entfernen?' % (COLOR2, latest), "Dieser wird beim Neustart wieder hergestellt[/COLOR]", nolabel='[B][COLOR darkgoldenrod]NEIN, abbrechen[/COLOR][/B]', yeslabel='[B][COLOR ghostwhite]JA, entfernen[/COLOR][/B]')
	if choice == 1:
